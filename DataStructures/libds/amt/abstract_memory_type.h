...
namespace ds::amt {
	...
	public:
		...
		virtual AbstractMemoryType& assign(const AbstractMemoryType& other) = 0;	// = 0; cista virtualna metoda (ako interface)
	...
	using AMT = AbstractMemoryType;	// (APT)
	...
	{
	public:
		AbstractMemoryStructure(mm::MemoryManager<BlockType>* memoryManager);	// potrebujeme mu dat SP
	...
	public:
		...
		// len volame operacie zo SP
		AMT& assign(const AMT& other) override;
		void clear() override;
		bool equals(const AMT& other) override;

		size_t getCapacity();
		void changeCapacity(size_t newCapacity);	// vhodne na zmensovanie (implicitna pamat struktura nam dovoli "upratat" po sebe)
	...
	{
	public:
		// kazda konkretna implementacia (sekvenice, hierarchie, siete) bude musiet definovat svoje operacie
	...
	// size() vrati POCET ALOKOVANYCH BLOKOV PAMATE
	template<typename BlockType>

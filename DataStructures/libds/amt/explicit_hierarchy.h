...
namespace ds::amt {
	...
	public:
		...
		// ustanovuje a vytvara koren (new koren, alokuj mu pamat)
    ...
    AMT& ExplicitHierarchy<BlockType>::assign(const AMT& other)
	{
		...
		copy = [&](BlockType* myBlock, BlockType* otherBlock)
		{
			...
			while (copiedSonCount < sonCount)
			{
				...
				if (otherSon != nullptr)
				{
					// nakopiruje do tohto syna obsah ineho syna
	...
	{
		// vieme efektivne vymazat hierarchiu vyuzitim spatnej prehliadky
		// najskor prejde co najdalej vie k synovi a az ked sa nemoze dostat na dalsieho syna, zavola operaciu vymazania
		// zabezpeci, ze vymazanie nesposobi, ze stratime odkaz na dalsie prvky, lebo ideme "odspodu"
		// [&] lebo potrebujeme poslat do lambdy memoryManager
		this->processPostOrder(root_, [&](BlockType* removed) { this->memoryManager_->releaseMemory(removed); });
		root_ = nullptr;
	}
	...
	// prekryjeme v potomkovi metodu isEmpty(), lebo dokazeme ju zjednodusit na O(1) tym, ze skontrolujeme, ci ma koren
	template<typename BlockType>
    ...
    bool ExplicitHierarchy<BlockType>::equals(const AMT& other)
	{
		...
		// compare prebera 2 bloky pamate
		// ulozime si ju ako lokalnu premennu
			// ta potom dokaze zavolat sama seba (rekurzia)
		// odstartujeme lambdu nad "mojim korenom", "inym korenom"
		...
		compare = [&](BlockType* myBlock, BlockType* otherBlock) -> bool
		{
			if (myBlock == nullptr && otherBlock == nullptr) {
				...
				// tu uz sme si isti, ze maju rovnaky stupen => mozeme spustit nad nimi for
	...
	{
		// v konstruktore sa vytvori prave taky memoryManager, ktory prebera za sablonovy typ konkretny typ bloku => tak vie, aky typ blokov ma alokovat
    ...
    MultiWayExplicitHierarchyBlock<DataType>& MultiWayExplicitHierarchy<DataType>::emplaceSon(BlockType& parent, size_t sonOrder)
	{
		...
		parent.sons_->insert(sonOrder).data_ = newSon;					// O(1), ak su sons_ implementovani ako IS a sonOrder je posledny

...
{
	return new (address) T();	// zavola implicitny konstruktor
}
 ...
 */
template<typename T>
T* placement_copy(T* address, const T& origin)	// zavola sa ale copy constructor
{
	return new (address) T(origin);
}

// aby fungoval heap monitor aj pri placement new

...
// uzemna jednotka
// atributy: nadradena jednotka, podradene jednotky
// alebo podedime nadradenu, podradene
// alebo kompoziciou: rovno data_; datova cast = uzemna jednotka
// algoritmus z 1.urovne
// vytvor iterator nad aktualnym vrcholom a spusti prehliadku
// jedina uloha: ako urobit nacitavanie; vsetko ostatne uz nakodovane na cvikach
...
namespace ds::adt {
    ...
    {
    public:
        // vypytame si cely blok (datovu aj vztahovu cast)
    ...
    private:
        ...
        HierarchyType* getHierarchy() const;                    // nad APS - hierarchiou len volame prislusne metody
    };
    ...
    template <typename T, typename HierarchyType>
    size_t GeneralTree<T, HierarchyType>::degree(const Node& node) const    // stupen param vrchola = pocet synov
    {
        return this->getHierarchy()->degree(this->getNode(node));
    }

    template <typename T, typename HierarchyType>
    size_t GeneralTree<T, HierarchyType>::nodeCount() const                 // mohutnost = pocet vrcholov celej hierarchie
    {
        return this->getHierarchy()->nodeCount();
    }

    template <typename T, typename HierarchyType>
    size_t GeneralTree<T, HierarchyType>::nodeCount(const Node& node) const // mohutnost podhierarchie s korenom v param vrchole
    {
        return this->getHierarchy()->nodeCount(this->getNode(node));
    }

    template <typename T, typename HierarchyType>
    typename GeneralTree<T, HierarchyType>::Node* GeneralTree<T, HierarchyType>::accessRoot() const                                 // spristupni koren
    {
        return this->getHierarchy()->accessRoot();
    }

    template <typename T, typename HierarchyType>
    typename GeneralTree<T, HierarchyType>::Node* GeneralTree<T, HierarchyType>::accessParent(const Node& node) const               // spristupni otca param vrchola
    {
        return this->getHierarchy()->accessParent(this->getNode(node));
    }

    template <typename T, typename HierarchyType>
    typename GeneralTree<T, HierarchyType>::Node* GeneralTree<T, HierarchyType>::accessSon(const Node& node, size_t sonOrder) const // spristuni n-teho (sonOrder) syna param vrchola
    {
    ...
    template <typename T, typename HierarchyType>
    typename GeneralTree<T, HierarchyType>::Node& GeneralTree<T, HierarchyType>::insertRoot()   // vloz koren do hierarchie
    {
        return this->getHierarchy()->emplaceRoot();
    }

    template <typename T, typename HierarchyType>
    void GeneralTree<T, HierarchyType>::changeRoot(Node* newRoot)                               // zmen koren na novy, poslany parametrom
    {
        this->getHierarchy()->changeRoot(this->getNode(newRoot));
    }

    template <typename T, typename HierarchyType>
    typename GeneralTree<T, HierarchyType>::Node& GeneralTree<T, HierarchyType>::emplaceSon(Node& parent, size_t sonOrder)  // vloz syna na n-te poradie u param otca
    {
        return this->getHierarchy()->emplaceSon(this->getNode(parent), sonOrder);
    }

    template <typename T, typename HierarchyType>
    void GeneralTree<T, HierarchyType>::changeSon(Node& parent, size_t sonOrder, Node* newSon)  // zmen syna na noveho (param vrchol) na n-tom poradi u otca
    {
        this->getHierarchy()->changeSon(this->getNode(parent), sonOrder, this->getNode(newSon));
    }

    template <typename T, typename HierarchyType>
    void GeneralTree<T, HierarchyType>::removeSon(Node& parent, size_t sonOrder)                // odstran n-teho syna u otca
    {
        this->getHierarchy()->removeSon(this->getNode(parent), sonOrder);
    }

    template <typename T, typename HierarchyType>
    bool GeneralTree<T, HierarchyType>::isRoot(const Node& node) const                          // je param vrchol korenom?
    {
        return this->getHierarchy()->isRoot(this->getNode(node));
    }

    template <typename T, typename HierarchyType>
    bool GeneralTree<T, HierarchyType>::isNThSon(const Node& node, size_t sonOrder) const       // je param vrchol n-tym (param order) synom?
    {
        return this->getHierarchy()->isNthSon(this->getNode(node), sonOrder);
    }

    template <typename T, typename HierarchyType>
    bool GeneralTree<T, HierarchyType>::isLeaf(const Node& node) const                          // je param vrchol listom?
    {
        return this->getHierarchy()->isLeaf(this->getNode(node));
    }

    template <typename T, typename HierarchyType>
    bool GeneralTree<T, HierarchyType>::hasNThSon(const Node& node, size_t sonOrder) const      // ma param vrchol n-teho syna?
    {
